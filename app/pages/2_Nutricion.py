...
ui.title("Nutrición Inteligente", "Escanea tu comida y registra en segundos")
...
with tab2:
    ...
    with st.form("nutrition_form"):
        date = st.date_input("Fecha", time_utils.local_today())
...
if not df.empty:
    ...
    if not today_df.empty:
        ...
        ui.kpi_grid(
            [
                {"label": "Calorías", "value": f"{int(total_cal)}", "sub": f"{int(remaining)} restantes"},
                {"label": "Proteína", "value": f"{int(total_pro)}g", "sub": "Objetivo diario"},
                {"label": "Carbs", "value": f"{int(total_carb)}g", "sub": "Objetivo diario"},
                {"label": "Grasas", "value": f"{int(total_fat)}g", "sub": "Objetivo diario"},
            ]
        )
        ui.card_end()
    ...
    with st.expander("📅 Ver resumen por día"):
        view_date = st.date_input("Día", today, key="nutrition_view_day")
        view_df = df[df['date'] == view_date]
        if not view_df.empty:
            view_cal = pd.to_numeric(view_df['calories'], errors='coerce').fillna(0).sum()
            view_pro = pd.to_numeric(view_df['protein'], errors='coerce').fillna(0).sum()
            view_carb = pd.to_numeric(view_df['carbs'], errors='coerce').fillna(0).sum()
            view_fat = pd.to_numeric(view_df['fats'], errors='coerce').fillna(0).sum()
            s1, s2, s3, s4 = st.columns(4)
            s1.metric("Calorías", int(view_cal))
            s2.metric("Proteína", f"{int(view_pro)}g")
            s3.metric("Carbs", f"{int(view_carb)}g")
            s4.metric("Grasas", f"{int(view_fat)}g")
        else:
